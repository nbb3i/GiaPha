...
/** Lấy toàn bộ thành viên huyết thống (không tính dâu/rể) đã hiển thị. */
export async function layTatCaThanhVien() {
  return prisma.person.findMany({
    where: { laDauRe: false, trangThai: "HIEN" },
    orderBy: [{ doi: "asc" }, { thuTu: "asc" }],
  });
}

/** Dựng cây từ danh sách phẳng dựa trên parentId. */
export function dungCay(list: Person[]): NodeCay[] {
  const map = new Map<string, NodeCay>();
  const roots: NodeCay[] = [];
...
/** Chi tiết một người kèm cha, con, vợ/chồng. */
export async function layChiTiet(id: string) {
  return prisma.person.findUnique({
    where: { id },
    include: {
      parent: true,
      children: { orderBy: { thuTu: "asc" } },
      media: true,
      honNhanChong: { include: { vo: true } },
      honNhanVo: { include: { chong: true } },
    },
  });
}
